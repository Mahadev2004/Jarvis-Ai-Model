import pygame # Import pygame library for handling audio palyback.
...
async def TextToAudioFile(text) -> None:
   ...
   if os.path.exists(file_path): # Check if theh file already exists.
   ...
   await communicate.save(r"Data\speech.mp3") # Save the generated spech as an MP3 file.
   
# Function to manage Text-to-Speech (TTS) functionality
def TTS(Text, func = lambda r = None: True):
   while True:
      try:
         # Convert text to an audio file asynchronuosly
         asyncio.run(TextToAudioFile(Text))

         # Initialize pygame mixer fro audio playback
         pygame.mixer.init()
         ...
         return True # Return True is the audio played successfully
         ...
         try:
            # Call the provided function with false to signal teh end of TTS
            func(False)
            pygame.mixer.music.stop() # Stop the audio palyback
...
def TextToSpeech(Text, func = lambda r = None: True):
   ...
   # Otherwise, jsut play the whole text
   else:
      TTS(Text, func)
...
if __name__ == "__main__":
   while True:
      # Prompt user for input and pass it to TextToSpeecch function
